...
int main()
{
    ...
    while(!feof(fp)) // till the end of the file
    {
        ...
        {
            fscanf(fp,"%s",m[j].macstr);// pointer is always incremented before a successful scan
            fscanf(fp,"%s",m[j].macval);// pointer is always incremented before a successful scan
